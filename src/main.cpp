...
#if defined(NDEBUG)
# error "PandoraCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe8b8fb6d1f5d40e89567d4ad95dc03cbcb68f308005577b6cc74def69a2fd9a6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PandoraCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PandoraCoin Signed Message:\n";
...
{
    // PandoraCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 250000); // PandoraCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 6*60; // PandoraCoin: every 2 block
static const int64 nTargetSpacing = 2 * 60; // PandoraCoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PandoraCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PandoraCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // PandoraCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "17/07/2014 MH17 flight was crash 298 people dies";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042A9D370C2FDCD218FFA27869766C9C8666F9E1060410A79BB02D37D84BF39DCC6527A62EBB80FF1EAC579EC264E20461CF1F27E3AB3DD9763E4E32F796BF0A2B") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1406667581;
        ...
        assert(block.hashMerkleRoot == uint256("0x6e65656d83660df439f6626f3a8f7f630b0e055205ea79068061fcbd08fde1cd"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // PandoraCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PandoraCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PandoraCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PandoraCoinMiner(CWallet *pwallet)
{
    printf("PandoraCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pandoracoin-miner");
    ...
    try { loop {
        ...
        printf("Running PandoraCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PandoraCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PandoraCoinMiner, pwallet));
}
